...
class CustomActionDistribution(dist.Distribution):
    """自定义动作分布处理混合动作空间"""
    ...
    def kl(self, other):
        return sum(dist.kl_divergence(d, other.components[k]) 
                for k, d in self.components.items())

class CustomActionModel(TorchModelV2, nn.Module):
    def __init__(self, obs_space, action_space, num_outputs, model_config, name):
        super().__init__(obs_space, action_space, num_outputs, model_config, name)
        
        # 共享特征编码部分（与之前观察空间处理相同）
        self.base_model = CustomObsModel(obs_space, action_space, num_outputs, model_config, name)
...
class TorchCustomDistribution(TorchDistributionWrapper):
    def __init__(self, inputs, model):
        ...
        self.dist = CustomActionDistribution(inputs)
