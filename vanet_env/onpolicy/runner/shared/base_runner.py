...
class Runner(object):
    ...
    def save_timestep(self, timestep):
        ...
        torch.save(
            ...
            str(self.save_dir) + "/actor" + str(timestep) + ".pt",
        )
        policy_critic = self.trainer.policy.critic
        torch.save(
            policy_critic.state_dict(),
            str(self.save_dir) + "/critic" + str(timestep) + ".pt",
        )
        if self.trainer._use_valuenorm:
            policy_vnorm = self.trainer.value_normalizer
            torch.save(
                policy_vnorm.state_dict(),
                str(self.save_dir) + "/vnorm" + str(timestep) + ".pt",
            )
    ...
    def restore_timestep(self, timestep):
        ...
        policy_actor_state_dict = torch.load(
            str(self.model_dir) + "/actor" + str(timestep) + ".pt"
        )
        self.policy.actor.load_state_dict(policy_actor_state_dict)
        if not self.all_args.use_render:
            policy_critic_state_dict = torch.load(
                str(self.model_dir) + "/critic" + str(timestep) + ".pt"
            )
            self.policy.critic.load_state_dict(policy_critic_state_dict)
            if self.trainer._use_valuenorm:
                policy_vnorm_state_dict = torch.load(
                    str(self.model_dir) + "/vnorm" + str(timestep) + ".pt"
                )
                self.trainer.value_normalizer.load_state_dict(policy_vnorm_state_dict)
