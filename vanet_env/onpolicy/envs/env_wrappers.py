...
class ShareDummyVecEnv(ShareVecEnv):
    ...
    def step(self, actions):
        results = [env.step(actions) for env in self.envs]
        ...
        for r_keys in r_obs:
            obs.append(r_obs[r_keys]["local_obs"])
            share_obs.append(r_obs[r_keys]["global_obs"])

        for r_keys in r_rew:
            rews.append(r_rew[r_keys])

        for r_keys in r_dones:
            dones.append(r_dones[r_keys])
